...
def read_parquet_AX3_epochs(path, subject_id, round_timestamps, subject_prefix=None):
    """
    This function reads Parquet files that contain one record per epoch
    ...
    which converts raw cwa files into these Parquet.
    
    Note: The reason for CWA to Parquet and then Parquet to Tensorflow is that reading the original
        CWA files is slower than reading Parquets. But Parquet are faster to read
    """
...
def read_AX3_cwa(path, subject_id, subject_prefix=None, freq=config['AX3_freq'], seconds_per_epoch=config['seconds_per_epoch']):
